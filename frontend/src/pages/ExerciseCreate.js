      ...
      <Card className="bg-zinc-900/50 backdrop-blur-md border-zinc-800">
        <CardContent className="p-6 space-y-4">
          <div>
            ...
            <Input data-testid="exercise-title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Les bases du reseau TCP/IP" className="bg-zinc-950 border-zinc-800 focus:border-cyan-500 text-zinc-100 placeholder:text-zinc-600" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select value={formation} onValueChange={(v) => { setFormation(v); setCategory(''); }}>
                <SelectTrigger data-testid="exercise-formation" className="bg-zinc-950 border-zinc-800 text-zinc-100">
                  <SelectValue />
                </SelectTrigger>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input data-testid="exercise-time-limit" type="number" value={timeLimit} onChange={(e) => setTimeLimit(e.target.value)} min={0} placeholder="0 = illimite" className="bg-zinc-950 border-zinc-800 focus:border-cyan-500 text-zinc-100 placeholder:text-zinc-600" />
            </div>
            <div className="flex items-end">
              <label className="flex items-center gap-2 cursor-pointer p-2">
                <input type="checkbox" checked={shared} onChange={(e) => setShared(e.target.checked)} className="rounded border-zinc-700 bg-zinc-950 text-cyan-500 focus:ring-cyan-500" />
                <span className="text-sm text-zinc-400">Partager entre formations</span>
              </label>
            </div>
          </div>
        </CardContent>
        ...
        {questions.map((q, idx) => (
          <Card key={q.id} className="bg-zinc-900/50 backdrop-blur-md border-zinc-800 animate-fade-in-up">
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Input type="number" value={q.points} onChange={(e) => updateQuestion(idx, 'points', e.target.value)} className="w-16 h-7 bg-zinc-950 border-zinc-800 text-zinc-100 text-xs text-center" min={1} />
                ...
                <div className="space-y-2">
                  ...
                  {q.options.map((opt, oIdx) => (
                    <div key={oIdx} className="flex items-center gap-2">
                      ...
                      <Input data-testid={`option-${idx}-${oIdx}`} value={opt} onChange={(e) => updateOption(idx, oIdx, e.target.value)} placeholder={`Option ${String.fromCharCode(65 + oIdx)}`} className="bg-zinc-950 border-zinc-800 focus:border-cyan-500 text-zinc-100 placeholder:text-zinc-600 flex-1" />
                    </div>
                  ))}
                  <Button size="sm" variant="ghost" className="text-xs text-zinc-400 hover:text-cyan-400" onClick={() => addOption(idx)}>+ Ajouter une option</Button>
                </div>
              )}
              <div>
                <label className="text-xs font-mono text-zinc-500 uppercase tracking-wider mb-2 block">{q.question_type === 'qcm' ? 'Reponse correcte' : "Reponse attendue (guide pour l'IA)"}</label>
                <Input data-testid={`correct-answer-${idx}`} value={q.correct_answer} onChange={(e) => updateQuestion(idx, 'correct_answer', e.target.value)} placeholder={q.question_type === 'qcm' ? 'Ex: DNS' : 'Elements de reponse attendus...'} className="bg-zinc-950 border-zinc-800 focus:border-cyan-500 text-zinc-100 placeholder:text-zinc-600" />
              </div>
            </CardContent>
          </Card>
        ))}
