...
import { BookOpen, Clock, ArrowRight, Server, Network, Shield, Cpu, Database, Terminal, PlusCircle } from 'lucide-react';
    ...
    <div className="space-y-6" data-testid="exercises-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500 mt-1">{exercises.length} exercice{exercises.length !== 1 ? 's' : ''} disponible{exercises.length !== 1 ? 's' : ''}</p>
        </div>
        {user?.role !== 'etudiant' && (
          <Button
          ...
          {categories.map((cat) => {
            ...
            return (
              <TabsTrigger
                key={cat.id}
                value={cat.id}
                className="data-[state=active]:bg-cyan-500/20 data-[state=active]:text-cyan-400 text-xs flex items-center gap-1"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((ex, i) => {
          const Icon = categoryIcons[ex.category] || BookOpen;
          const isCompleted = completedIds.has(ex.id);
          return (
            <Card
              key={ex.id}
              className="group relative overflow-hidden bg-zinc-900 border border-zinc-800 hover:border-cyan-500/50 transition-all duration-300 hover:shadow-[0_0_20px_rgba(6,182,212,0.1)] cursor-pointer animate-fade-in-up"
            ...
            >
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {isCompleted && (
                    <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px]">Complete</Badge>
                  )}
                </div>
                <h3 className="text-sm font-semibold text-zinc-200 group-hover:text-cyan-400 transition-colors mb-1" style={{ fontFamily: 'Space Grotesk' }}>
                  {ex.title}
                </h3>
                ...
                <div className="flex items-center gap-3 text-xs text-zinc-500">
                  ...
                  {ex.time_limit > 0 && (
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {ex.time_limit}min</span>
                  )}
                  <span>{ex.submission_count} soumission{ex.submission_count !== 1 ? 's' : ''}</span>
                </div>
                {user?.role !== 'etudiant' && (
                  <div className="mt-3 pt-3 border-t border-zinc-800 flex gap-2">
                    <Button size="sm" variant="ghost" className="text-xs text-zinc-400 hover:text-cyan-400 h-7" onClick={(e) => { e.stopPropagation(); navigate(`/exercises/edit/${ex.id}`); }}>
                      Modifier
                    </Button>
                    <Button size="sm" variant="ghost" className="text-xs text-red-400 hover:text-red-300 h-7" onClick={(e) => { e.stopPropagation(); handleDelete(ex.id); }}>
                      Supprimer
                    </Button>
                  </div>
                )}
