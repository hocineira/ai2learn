    ...
    <div className="space-y-6" data-testid="exercises-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-zinc-500 mt-1">{formationLabel} - {exercises.length} exercice{exercises.length !== 1 ? 's' : ''}</p>
        </div>
        {user?.role !== 'etudiant' && (
          ...
          return (
            <Card key={ex.id} className="group relative overflow-hidden bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 hover:border-cyan-500/50 transition-all duration-300 cursor-pointer animate-fade-in-up shadow-sm dark:shadow-none"
              ...
              <CardContent className="p-5">
                ...
                <h3 className="text-sm font-semibold text-gray-800 dark:text-zinc-200 group-hover:text-cyan-400 transition-colors mb-1" style={{ fontFamily: 'Space Grotesk' }}>{ex.title}</h3>
                <p className="text-xs text-gray-500 dark:text-zinc-500 line-clamp-2 mb-3">{ex.description}</p>
                ...
                {user?.role !== 'etudiant' && (
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-zinc-800 flex gap-2">
                    <Button size="sm" variant="ghost" className="text-xs text-gray-500 dark:text-zinc-400 hover:text-cyan-400 h-7" onClick={(e) => { e.stopPropagation(); navigate(`/exercises/edit/${ex.id}`); }}>Modifier</Button>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          <BookOpen className="w-12 h-12 text-gray-300 dark:text-zinc-700 mx-auto mb-3" />
          <p className="text-gray-500 dark:text-zinc-500">Aucun exercice dans cette categorie</p>
        </div>
      )}
    </div>
  );
