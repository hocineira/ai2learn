...
export default function UsersPage() {
  const { getAuthHeaders, API, user: currentUser } = useAuth();
  ...
  const updateRole = async (userId, newRole) => {
    try {
      await axios.put(`${API}/users/${userId}/role`, { role: newRole }, { headers: getAuthHeaders() });
  ...
  return (
    <div className="space-y-6" data-testid="users-page">
      <div>
        ...
        <p className="text-zinc-500 mt-1">{users.length} utilisateur{users.length !== 1 ? 's' : ''} enregistre{users.length !== 1 ? 's' : ''}</p>
      </div>
                  ...
                  <TableCell>
                    {currentUser?.role === 'admin' && u.id !== currentUser.id ? (
                      <Select value={u.role} onValueChange={(val) => updateRole(u.id, val)}>
                        <SelectTrigger className="w-36 h-7 bg-zinc-950 border-zinc-800 text-xs">
                          <SelectValue />
                        </SelectTrigger>
