...
export default function TrackingPage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get(`${API}/stats/students-tracking`, { headers: getAuthHeaders() });
  ...
  return (
    <div className="space-y-6" data-testid="tracking-page">
      <div>
        ...
        <p className="text-zinc-500 mt-1">Progression en temps reel de vos etudiants</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {tracking.map((student, i) => (
          <Card
            key={student.id}
            className="bg-zinc-900/50 backdrop-blur-md border-zinc-800 hover:border-emerald-500/30 transition-all animate-fade-in-up"
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-bold" style={{ color: student.avg_score >= 50 ? '#10b981' : student.avg_score > 0 ? '#f59e0b' : '#71717a' }}>
                      {student.avg_score}%
                    </span>
                  </div>
                  <Progress
                    value={student.avg_score}
                    className={`h-1.5 bg-zinc-800 ${student.avg_score >= 50 ? '[&>div]:bg-emerald-500' : '[&>div]:bg-amber-500'}`}
                  />
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-zinc-500">Derniere activite</span>
                  <span className="text-xs text-zinc-400">
                    {student.last_activity ? new Date(student.last_activity).toLocaleDateString('fr-FR') : '-'}
                  </span>
                </div>
              </div>
            </CardContent>
      ...
      {tracking.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-zinc-500">Aucun etudiant inscrit</p>
        </div>
      )}
    </div>
  );
